...
public class HistoricoMedicoController {
    ...
    public Flux<HistoricoMedico> getHistoricosPorUsuario(String idUsuario){
        return historicoMedicoService.findByIdUsuario(idUsuario);
    }

    @PutMapping(value = "/alterar")
    @ApiOperation(value = "Altera um histórico médico", response = HistoricoMedico.class, notes = "O endpoint requer token de autenticação. Deve ser passado no body " +
    "da requisição um objeto JSON contendo os dados do hitórico médico e seu respectivo ID, todos os campos são obrigatórios.")
    public Mono<HistoricoMedico> putHistorico(@RequestBody HistoricoMedico historico){
        return historicoMedicoService.save(historico);
    }

    @PostMapping(value = "/salvar")
    @ApiOperation(value="Salva um histórico médico", response = HistoricoMedico.class, notes = "O endpoint requer token de autenticação. Deve ser passado no body " +
